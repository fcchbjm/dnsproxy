...
var commandLineOptions = []*commandLineOption{
	...
	proxyProtocolV2ReadTimeoutIdx: {
		description: "Timeout for reading Proxy Protocol v2 headers and payload on new TCP/TLS connections.",
		long:        "proxy-protocol-v2-read-timeout",
		short:       "",
		valueType:   "duration",
	},
	dnssecEnabledIdx: {
...
func parseCmdLineOptions(conf *configuration) (err error) {
	...
	for i, fieldPtr := range []any{
		...
		proxyProtocolV2ReadTimeoutIdx: &conf.ProxyProtocolV2ReadTimeout,
	} {
